 ...
 *
 * Проверка не смотрит в исходники: она сравнивает два ГОТОВЫХ контракта, собранных
 * РАЗНЫМИ парсерами из разных источников — `api.json` из разметки эталона,
 * `api.react.json` из TypeScript.
 *
 * ГРАНИЦА, которую надо знать: сеть натянута только над пропсами, которые есть у ЭТАЛОНА.
 * `api.json` — снимок замороженного тега, расти он не может, поэтому проп, рождённый в
 * React-версии, второго разбора не имеет и этой сверкой не покрыт (сегодня таких 41).
 * Их держит другое — юнит-тест `tests/unit/tsx-parser.test.mjs`, который перечисляет
 * законные формы записи и требует, чтобы разбор видел каждую. Раньше здесь стояло
 * «спрятаться ему негде»; для новой работы это было неправдой.
 *
