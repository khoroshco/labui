 ...
 * что уезжает потребителю, и нужен витрине: плейграунд показывает СОБСТВЕННЫЕ пропсы компонента.
...
function optionsOf(type) {
  // Ведущая черта — дефолтный вывод prettier для многострочного union. Раньше первый
  // кусок разреза оказывался пустым, проверка «все куски — строковые литералы» падала,
  // и enum молча превращался в null: проп оставался, а переключатель вариантов в
  // плейграунде исчезал.
  type = String(type).replace(/^\s*\|/, '').trim();
