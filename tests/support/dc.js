 ...
 * вместе с DC-рантаймом, когда витрина переедет на React), а оставить как 404 — значит держать гейт вечно красным.
...
export function preparePage(page) {
  ...
  // Счётчик запросов В ПОЛЁТЕ. Рантайм эталона тянет каждый вложенный компонент и каждую
  // иконку отдельным запросом, поэтому дерево собирается волнами. «Тишина DOM» между
  // волнами обманчива: пауза бывает длиннее окна тишины, и оснастка считала дерево
  // достроенным на половине. Ждать нужно ДВУХ фактов сразу — никто ничего не грузит И
  // дерево не меняется. Второй факт без первого — это ожидание паузы, а не конца работы.
  let inflight = 0;
